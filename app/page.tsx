    ...
    <main className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 space-y-20 py-12">
        ...
        <section className="grid md:grid-cols-3 gap-6 px-4">
          {[
            { icon: '⚔️', title: 'Grid Combat', desc: 'Command your units on a 3x3 battlefield', color: 'from-red-500 to-orange-500' },
            { icon: '⛓️', title: 'On-Chain', desc: 'Every move verified on Base blockchain', color: 'from-cyan-500 to-blue-500' },
            { icon: '📱', title: 'Frames', desc: 'Play in Farcaster feed, no downloads', color: 'from-purple-500 to-pink-500' },
            { icon: '🚀', title: 'Free Gas', desc: 'Meta-transactions sponsor your battles', color: 'from-green-500 to-emerald-500' },
            { icon: '🏅', title: 'Ranked', desc: 'Climb leaderboards, win tournaments', color: 'from-yellow-500 to-orange-500' },
            { icon: '💰', title: 'Earn Crypto', desc: 'Win real rewards on Base mainnet', color: 'from-indigo-500 to-purple-500' }
          ].map((feature, i) => (
            <div
              key={i}
              className="group relative p-6 rounded-2xl border border-slate-700/50 bg-slate-900/50 backdrop-blur-sm hover:bg-slate-800/50 transition-all duration-300 hover:scale-105 hover:-translate-y-2 cursor-pointer animate-fade-in"
        ...
        <section className="py-12 px-6">
          <div className="text-center mb-12">
            <h2 className="text-4xl md:text-5xl font-black bg-gradient-to-r from-cyan-400 to-indigo-400 bg-clip-text text-transparent mb-4">
            ...
            {units.map((unit, i) => (
              ...
              >
                ...
                <div className="relative z-10">
                  {/* Unit Icon */}
                  <div className="text-8xl mb-6 text-center group-hover:scale-110 transition-transform duration-300 filter drop-shadow-2xl">
                    {unit.emoji}
                  </div>
        ...
        <section className="relative py-12 px-6 rounded-3xl border border-cyan-500/30 bg-gradient-to-br from-slate-900/80 to-indigo-900/50 backdrop-blur-sm overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6 max-w-6xl mx-auto">
              {[
                { value: '6', label: 'Smart Contracts', icon: '📜', color: 'cyan' },
                { value: '160', label: 'Tests Passing', icon: '✅', color: 'green' },
                { value: '10+', label: 'API Endpoints', icon: '🔌', color: 'purple' },
                { value: '32', label: 'Git Commits', icon: '💾', color: 'orange' },
                { value: '100%', label: 'Uptime', icon: '⚡', color: 'yellow' }
              ].map((stat, i) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <div className={`text-4xl font-black mb-1 bg-gradient-to-r from-${stat.color}-400 to-${stat.color}-600 bg-clip-text text-transparent`}>
                    {stat.value}
                  </div>
        ...
        <section className="py-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: '📘', title: 'README', desc: 'Setup & quick start', link: 'README.md', color: 'blue' },
              { icon: '📗', title: 'Contracts', desc: 'ABI & function specs', link: 'CONTRACTS.md', color: 'green' },
              { icon: '📙', title: 'Frames', desc: 'Farcaster integration', link: 'FRAMES.md', color: 'yellow' },
              { icon: '🧪', title: 'Testing', desc: 'Manual scenarios', link: 'TESTING.md', color: 'purple' },
              { icon: '📕', title: 'Deployment', desc: 'Deploy to mainnet', link: 'DEPLOYMENT.md', color: 'red' },
              { icon: '🏆', title: 'Contest', desc: 'Feature overview', link: 'CONTEST_SUMMARY.md', color: 'cyan' }
            ].map((doc, i) => (
              <a
                key={i}
              ...
              >
                <div className={`absolute inset-0 bg-gradient-to-br from-${doc.color}-500/0 to-${doc.color}-500/0 group-hover:from-${doc.color}-500/10 group-hover:to-${doc.color}-500/5 transition-all`}></div>
